  ...
  const formatMessage = (content: string) => {
    ...
    return parts.map((part, index) => {
      if (part.startsWith('```')) {
        ...
        return (
          <div key={index} className="my-4 rounded-xl overflow-hidden border border-gray-700 hover:border-blue-500/30 transition-colors">
            <div className="flex items-center justify-between bg-gradient-to-r from-gray-800 to-gray-800 px-4 py-3 border-b border-gray-700">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-700/50 hover:bg-gray-700 text-gray-400 hover:text-white transition-all text-xs"
              >
                {copiedId === `code-${index}` ? (
                  <>
        ...
        <span key={index}>
          {textParts.map((textPart, i) => {
            if (textPart.startsWith('`') && textPart.endsWith('`')) {
              return (
                <code key={i} className="bg-gray-800 text-blue-400 px-1.5 py-0.5 rounded text-sm font-mono">
                  {textPart.slice(1, -1)}
                </code>
              );
            }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
        ...
        <div className="max-w-4xl mx-auto space-y-6">
                ...
                >
                  ...
                  {message.role === 'assistant' && message.context && message.context !== 'general' && (
                    <div className="flex items-center gap-2 mb-2 pb-2 border-b border-gray-700">
                      ...
                      <span className="text-xs text-gray-400">{getContextLabel(message.context)}</span>
                    </div>
                  )}
          ...
          {isLoading && (
            <div className="flex gap-4 message-assistant">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center animate-pulse shadow-lg shadow-blue-500/50">
                <Sparkles className="w-4 h-4 text-white" />
              </div>
              <div className="bg-gradient-to-r from-gray-800 to-gray-800 rounded-2xl p-5 border border-blue-500/20 shadow-xl">
                <div className="flex gap-2 items-center">
                  <div className="w-2.5 h-2.5 bg-blue-500 rounded-full typing-dot shadow-lg shadow-blue-500/50" />
                  <div className="w-2.5 h-2.5 bg-purple-500 rounded-full typing-dot shadow-lg shadow-purple-500/50" />
                  <div className="w-2.5 h-2.5 bg-pink-500 rounded-full typing-dot shadow-lg shadow-pink-500/50" />
                  <span className="ml-2 text-xs text-gray-400">TAI is thinking...</span>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-gray-900 via-gray-900 to-transparent">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="bg-gradient-to-r from-gray-800 to-gray-800 rounded-2xl border border-blue-500/20 shadow-2xl hover:border-blue-500/40 transition-all duration-300">
            <div className="flex items-end gap-3 p-4">
              <textarea
                ...
                className="flex-1 bg-transparent text-white placeholder-gray-500 resize-none outline-none min-h-[24px] max-h-[200px] py-2 px-2 text-base"
                rows={1}
              />
              <button
                onClick={handleSend}
                disabled={!input.trim() || isLoading}
                className="relative group flex-shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br from-blue-600 to-purple-600 text-white flex items-center justify-center hover:from-blue-500 hover:to-purple-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-blue-500/50 transform hover:scale-105"
              >
